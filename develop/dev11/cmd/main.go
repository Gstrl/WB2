...
import (
	...
	"WB2/develop/dev11/internal/transport"
	"log"
	"os"
	"os/signal"
	"syscall"
...
func main() {
	...
	// 2) Инициализация cache
	cache := database.NewCacheEventRep()

	// 3) Запуск веб-сервера
	go func() {
		err := transport.ServerRun(cfg, cache)
		if err != nil {
			log.Fatalf("Ошибка запуска веб-сервера: %v", err)
		}
	}()
	log.Println("Веб-сервер успешно запущен")
	// 4) Ожидание сигнала для завершения работы приложения
	signalCh := make(chan os.Signal, 1)
	signal.Notify(signalCh, syscall.SIGINT, syscall.SIGTERM)
	<-signalCh

	log.Println("HTTPCalendar завершает работу.")
}
